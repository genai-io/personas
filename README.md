...
| `software-engineer` | A disciplined senior engineer: thinks before coding, asks instead of assuming, writes the minimum that solves the problem, changes only what the task requires. Bundles four skills — `code-review` (eleven angles over correctness, cleanup, and design, every finding verified before it is reported), `refactor` (behavior-preserving restructuring), `debug` (root-cause-first), `tdd` (red/green/refactor). Principles from [Andrej Karpathy's observations](https://x.com/karpathy/status/2015883857489522876) on LLM coding pitfalls. |
