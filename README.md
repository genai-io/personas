...
| `software-engineer` | A disciplined senior engineer: thinks before coding, asks instead of assuming, writes the minimum that solves the problem, changes only what the task requires. Bundles six skills — `code-review` (bugs and cleanups in one verified pass over the diff), `refactor` (behavior-preserving restructuring), `debug` (root-cause-first), `tdd` (red/green/refactor), `naming` (vocabulary consistency, names that match what they do), `overdesign` (abstractions, layers, and generality the problem never asked for). Principles from [Andrej Karpathy's observations](https://x.com/karpathy/status/2015883857489522876) on LLM coding pitfalls. |
