...
| `software-engineer` | A disciplined senior engineer: thinks before coding, asks instead of assuming, writes the minimum that solves the problem, changes only what the task requires. Bundles five skills — `code-review` (bugs), `refactor` (behavior-preserving restructuring), `debug` (root-cause-first), `tdd` (red/green/refactor), `simplify` (quality/reuse/efficiency cleanup). Principles from [Andrej Karpathy's observations](https://x.com/karpathy/status/2015883857489522876) on LLM coding pitfalls. |
